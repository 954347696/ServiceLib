package com.keepfun.imageselector.view;
...
public class CropImageView extends AppCompatImageView {
    ...
    private int mFocusWidth = 400;         //焦点框的宽度
    ...
    public CropImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.IcsCropImageView);
        mMaskColor = a.getColor(R.styleable.IcsCropImageView_icsCropMaskColor, mMaskColor);
        mBorderColor = a.getColor(R.styleable.IcsCropImageView_icsCropBorderColor, mBorderColor);
        mBorderWidth = a.getDimensionPixelSize(R.styleable.IcsCropImageView_icsCropBorderWidth, mBorderWidth);
        mFocusWidth = a.getDimensionPixelSize(R.styleable.IcsCropImageView_icsCropFocusWidth, mFocusWidth);
        mFocusHeight = a.getDimensionPixelSize(R.styleable.IcsCropImageView_icsCropFocusHeight, mFocusHeight);
        mDefaultStyleIndex = a.getInteger(R.styleable.IcsCropImageView_icsCropStyle, mDefaultStyleIndex);
...
//                    bitmap.recycle();
     ...
     * @param isSaveRectangle 是否希望按矩形区域保存图片
     */
    public void saveBitmapToFile(File folder, boolean isSaveRectangle) {
        if (mSaving) return;
        mSaving = true;
        final Bitmap croppedImage = getCropBitmap(isSaveRectangle ? mFocusWidth : mFocusHeight, mFocusHeight, isSaveRectangle);
        Bitmap.CompressFormat outputFormat = Bitmap.CompressFormat.JPEG;
        File saveFile = createFile(folder, "IMG_", ".jpg");
        if (mStyle == Style.CIRCLE && !isSaveRectangle) {
            outputFormat = Bitmap.CompressFormat.JPEG;
            saveFile = createFile(folder, "IMG_", ".jpg");
        }
        final Bitmap.CompressFormat finalOutputFormat = outputFormat;
        final File finalSaveFile = saveFile;
        new Thread() {
