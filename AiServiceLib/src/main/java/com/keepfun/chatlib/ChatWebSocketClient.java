...
public class ChatWebSocketClient extends ChatWebSocketTransport {
    ...
    private boolean scheduleReconnect() {
        ...
        ChatLogger.d(TAG, "scheduleReconnect() ");
        mHandler.postDelayed(() -> {
                    if (mClosed) {
                        return;
                    }
                    ChatLogger.w(TAG, "doing reconnect job, retryCount: " + mRetryStrategy.retryCount);
    ...
    private class ProtooWebSocketListener extends WebSocketListener {
        ...
        public void onClosing(@NotNull WebSocket webSocket, int code, @NotNull String reason) {
            ChatLogger.w(TAG, "onClosing()");
        }

        @Override
        public void onFailure(@NotNull WebSocket webSocket, @NotNull Throwable t, @Nullable Response response) {
            ChatLogger.w(TAG, "onFailure()");
            if (mClosed) {
                return;
            }
            if (scheduleReconnect()) {
                ...
                ChatLogger.e(TAG, "give up reconnect. notify closed");
                mClosed = true;
                if (mListener != null) {
                    mListener.onClose();
                }
        ...
        public void onMessage(@NotNull WebSocket webSocket, @NotNull String text) {
            ...
            ChatMessage message = ChatMessage.parse(text);
            if (message == null) {
                return;
            }
            if (mListener != null) {
    ...
    private OkHttpClient getUnsafeOkHttpClient() {
        try {
            ...
            HttpLoggingInterceptor httpLoggingInterceptor = new HttpLoggingInterceptor(s -> ChatLogger.d(TAG, s));
