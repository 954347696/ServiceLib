package com.keepfun.glide.progress;
...
import com.keepfun.glide.util.Utils;
...
public class CircleProgressView extends ProgressBar {
    ...
    private void obtainAttributes(AttributeSet attrs) {
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.IcsCircleProgressView);
        mProgressStyle = ta.getInt(R.styleable.IcsCircleProgressView_ics_cpv_progressStyle, ProgressStyle.NORMAL);
        // 获取三种风格通用的属性
        mNormalBarSize = (int) ta.getDimension(R.styleable.IcsCircleProgressView_ics_cpv_progressNormalSize, mNormalBarSize);
        mNormalBarColor = ta.getColor(R.styleable.IcsCircleProgressView_ics_cpv_progressNormalColor, mNormalBarColor);

        mReachBarSize = (int) ta.getDimension(R.styleable.IcsCircleProgressView_ics_cpv_progressReachSize, mReachBarSize);
        mReachBarColor = ta.getColor(R.styleable.IcsCircleProgressView_ics_cpv_progressReachColor, mReachBarColor);

        mTextSize = (int) ta.getDimension(R.styleable.IcsCircleProgressView_ics_cpv_progressTextSize, mTextSize);
        mTextColor = ta.getColor(R.styleable.IcsCircleProgressView_ics_cpv_progressTextColor, mTextColor);
        mTextSkewX = ta.getDimension(R.styleable.IcsCircleProgressView_ics_cpv_progressTextSkewX, 0);
        if (ta.hasValue(R.styleable.IcsCircleProgressView_ics_cpv_progressTextSuffix)) {
            mTextSuffix = ta.getString(R.styleable.IcsCircleProgressView_ics_cpv_progressTextSuffix);
        }
        if (ta.hasValue(R.styleable.IcsCircleProgressView_ics_cpv_progressTextPrefix)) {
            mTextPrefix = ta.getString(R.styleable.IcsCircleProgressView_ics_cpv_progressTextPrefix);
        }
        mTextVisible = ta.getBoolean(R.styleable.IcsCircleProgressView_ics_cpv_progressTextVisible, mTextVisible);

        mRadius = (int) ta.getDimension(R.styleable.IcsCircleProgressView_ics_cpv_radius, mRadius);
        ...
        switch (mProgressStyle) {
            ...
            case ProgressStyle.FILL_IN_ARC:
                mStartArc = ta.getInt(R.styleable.IcsCircleProgressView_ics_cpv_progressStartArc, 0) + 270;
                mInnerPadding = (int) ta.getDimension(R.styleable.IcsCircleProgressView_ics_cpv_innerPadding, mInnerPadding);
                mOuterColor = ta.getColor(R.styleable.IcsCircleProgressView_ics_cpv_outerColor, mReachBarColor);
                mOuterSize = (int) ta.getDimension(R.styleable.IcsCircleProgressView_ics_cpv_outerSize, mOuterSize);
                mReachBarSize = 0;// 将画笔大小重置为0
                mNormalBarSize = 0;
                if (!ta.hasValue(R.styleable.IcsCircleProgressView_ics_cpv_progressNormalColor)) {
            ...
            case ProgressStyle.NORMAL:
                mReachCapRound = ta.getBoolean(R.styleable.IcsCircleProgressView_ics_cpv_reachCapRound, true);
                mStartArc = ta.getInt(R.styleable.IcsCircleProgressView_ics_cpv_progressStartArc, 0) + 270;
                if (ta.hasValue(R.styleable.IcsCircleProgressView_ics_cpv_innerBackgroundColor)) {
                    mInnerBackgroundColor = ta.getColor(R.styleable.IcsCircleProgressView_ics_cpv_innerBackgroundColor, Color.argb(0, 0, 0, 0));
