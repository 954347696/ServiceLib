package com.keepfun.aiservice.ui.view.ratingbar;
...
//                R.styleable.AndRatingBar, defStyleAttr, 0);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.IcsAndRatingBar, defStyleAttr, 0);
        right2Left = typedArray.getBoolean(R.styleable.IcsAndRatingBar_icsRight2Left, false);

        if (typedArray.hasValue(R.styleable.IcsAndRatingBar_icsStarColor)) {
            if (right2Left) {
                mBgColor = typedArray.getColorStateList(
                        R.styleable.IcsAndRatingBar_icsStarColor);
            } else {
        ...
        if (typedArray.hasValue(R.styleable.IcsAndRatingBar_icsSubStarColor)) {
            if (!right2Left) {
                mSubStarColor = typedArray.getColorStateList(
                        R.styleable.IcsAndRatingBar_icsSubStarColor);
            }
        }

        if (typedArray.hasValue(R.styleable.IcsAndRatingBar_icsBgColor)) {
            if (right2Left) {
                mStarColor = typedArray.getColorStateList(
                        R.styleable.IcsAndRatingBar_icsBgColor);
            } else {
                mBgColor = typedArray.getColorStateList(
                        R.styleable.IcsAndRatingBar_icsBgColor);
            }
        ...
        mKeepOriginColor = typedArray.getBoolean(R.styleable.IcsAndRatingBar_icsKeepOriginColor, false);
        scaleFactor = typedArray.getFloat(R.styleable.IcsAndRatingBar_icsScaleFactor, 1);
        starSpacing = typedArray.getDimension(R.styleable.IcsAndRatingBar_icsStarSpacing, 0);

        // get customize drawable
        mStarDrawable = typedArray.getResourceId(R.styleable.IcsAndRatingBar_icsStarDrawable, R.drawable.ic_rating_star_solid);
        if (typedArray.hasValue(R.styleable.IcsAndRatingBar_icsBgDrawable)) {
            mBgDrawable = typedArray.getResourceId(R.styleable.IcsAndRatingBar_icsBgDrawable, R.drawable.ic_rating_star_solid);
        } else {
