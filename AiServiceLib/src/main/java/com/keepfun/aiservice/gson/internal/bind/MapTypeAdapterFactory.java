...
import com.keepfun.aiservice.gson.JsonPrimitive;
import com.keepfun.aiservice.gson.JsonSyntaxException;
...
import com.keepfun.aiservice.gson.internal.JsonReaderInternalAccess;
 ...
 *   System.out.println(EGson.toJson(original, type));
 * }</pre>
 ...
 * <pre>com.google.EGson.JsonParseException: Expecting object found: "(5,6)"
 *   at com.google.EGson.JsonObjectDeserializationVisitor.visitFieldUsingCustomHandler
 *   at com.google.EGson.ObjectNavigator.navigateClassFields
 *   ...</pre>
 *
...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
  ...
  public <T> TypeAdapter<T> create(EGson EGson, TypeToken<T> typeToken) {
    ...
    TypeAdapter<?> keyAdapter = getKeyAdapter(EGson, keyAndValueTypes[0]);
    TypeAdapter<?> valueAdapter = EGson.getAdapter(TypeToken.get(keyAndValueTypes[1]));
    ObjectConstructor<T> constructor = constructorConstructor.get(typeToken);

    @SuppressWarnings({"unchecked", "rawtypes"})
    // we don't define a type parameter for the key or value types
    TypeAdapter<T> result = new Adapter(EGson, keyAndValueTypes[0], keyAdapter,
   ...
   */
  private TypeAdapter<?> getKeyAdapter(EGson context, Type keyType) {
  ...
  private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
    ...
    public Adapter(EGson context, Type keyType, TypeAdapter<K> keyTypeAdapter,
