 ...
 * By default EGson converts application classes to JSON using its built-in type
 * adapters. If EGson's default JSON conversion isn't appropriate for a type,
 ...
 *     }
 *   }}</pre>
 * With this type adapter installed, EGson will convert {@code Points} to JSON as
 ...
 * registering the type adapter with EGson. If your {@code EGson} instance
 ...
 *
 * <p>To use a custom type adapter with EGson, you must <i>register</i> it with a
...
// <p>A type adapter registered with EGson is automatically invoked while serializing
...
public abstract class TypeAdapter<T> {
   ...
   * Unlike EGson's similar {@link EGson#toJson(JsonElement, Appendable) toJson}
   ...
   * <pre>   {@code
   *
   * EGson gson = new GsonBuilder().registerTypeAdapter(Foo.class,
   *   new TypeAdapter<Foo>() {
  ...
  /**
   * Converts {@code value} to a JSON document. Unlike EGson's similar {@link
   * EGson#toJson(Object) toJson} method, this write is strict. Create a {@link
   * JsonWriter#setLenient(boolean) lenient} {@code JsonWriter} and call
   * {@link #write(JsonWriter, Object)} for lenient
   * writing.
   *
  ...
  /**
   * Converts the JSON document in {@code in} to a Java object. Unlike EGson's
   * similar {@link EGson#fromJson(Reader, Class) fromJson} method, this
  ...
  /**
   * Converts the JSON document in {@code json} to a Java object. Unlike EGson's
   * similar {@link EGson#fromJson(String, Class) fromJson} method, this read is
