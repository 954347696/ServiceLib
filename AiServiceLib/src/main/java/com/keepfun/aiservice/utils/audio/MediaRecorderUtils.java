...
public class MediaRecorderUtils {
    ...
    private void getDecibel() {
        ...
        if (mMediaRecorderCallBack != null) {
            mMediaRecorderCallBack.decibel((int) decibel);
        }
    }
    ...
    public void start() {
        ...
        if (!isRecording) {
            try {
                ...
                if (mMediaRecorderCallBack != null) {
                    mMediaRecorderCallBack.stop(mSecond);
    ...
    private String getExternalDir() {
        //新建录音存储的文件夹
        String path = Utils.getApp().getExternalCacheDir().getPath() + "/recorder";
